...
local function encode(s, filename_safe)
	...
	for i = 1, #s, 3 do
		...
		c2 = byte(s,i+1) or 0
		c3 = byte(s,i+2) or 0
		t4[1] = char(byte(b64chars, bit.rshift(c1, 2) + 1))
		t4[2] = char(byte(b64chars, bit.band(bit.bor(bit.lshift(c1, 4), bit.rshift(c2, 4)), 0x3f) + 1))
		t4[3] = char(byte(b64chars, bit.band(bit.bor(bit.lshift(c2, 2), bit.rshift(c3, 6)), 0x3f) + 1))
		t4[4] = char(byte(b64chars, bit.band(c3, 0x3f) + 1))
		st[#st+1] = concat(t4)
